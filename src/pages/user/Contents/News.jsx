...
function News() {
  ...
  return (
    <div className="text-black font-light mx-4 md:mx-8 lg:mx-16 mt-8 mb-12">
      ...
      <div className="mb-4 z-10"> {/* Add z-index here */}
        <input
          type="text"
      ...
      {isModalOpen && selectedNews && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
          <div ref={modalRef} className="bg-white p-6 md:p-8 lg:p-12 rounded-lg max-w-full md:max-w-3xl lg:max-w-4xl w-full h-auto overflow-y-auto max-h-full relative">
            ...
            <div className="text-2xl font-medium mb-2">{selectedNews.title}</div>
            <div className="text-md mb-2">{selectedNews.date}</div>
            <img src={selectedNews.image} alt={selectedNews.title} className="mb-4 w-full h-48 md:h-64 lg:h-80 object-cover rounded" />
            <div className="text-sm mb-4">{selectedNews.description}</div>
            <div className="text-sm font-medium mb-2">{selectedNews.contact}</div>
            <a href={`mailto:news@example.com`} className="block text-sm text-blue-600 underline">news@example.com</a>
          </div>
        </div>
      )}
    </div>
