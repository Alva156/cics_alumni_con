      ...
      {isModalOpen && selectedEvent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
          <div ref={modalRef} className="bg-white p-6 md:p-8 lg:p-12 rounded-lg max-w-full md:max-w-3xl lg:max-w-4xl w-full h-auto overflow-y-auto max-h-full relative">
            ...
            <div className="text-2xl font-medium mb-2">{selectedEvent.title}</div>
            <div className="text-md mb-2">{selectedEvent.date}</div>
            <img src={selectedEvent.image} alt={selectedEvent.title} className="mb-4 w-full h-48 md:h-64 lg:h-80 object-cover rounded" />
            <div className="text-sm mb-4">{selectedEvent.description}</div>
            <div className="text-sm font-medium mb-2">{selectedEvent.contact}</div>
            <a href={`mailto:events@example.com`} className="block text-sm text-blue-600 underline">events@example.com</a>
          </div>
        </div>
      )}
    </div>
